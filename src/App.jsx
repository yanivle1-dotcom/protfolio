    ...
    <div dir={isRTL ? 'rtl' : 'ltr'} className={`${isDark ? 'bg-slate-950 text-slate-100' : 'bg-[#FDFBF9] text-slate-800'} min-h-screen font-sans transition-colors duration-500 selection:bg-slate-200 selection:text-black`}>
      <style>
        {`
          ...
          .font-sans { font-family: 'Assistant', sans-serif; }
          .smooth-reveal { animation: reveal 0.8s cubic-bezier(0.16, 1, 0.3, 1) forwards; opacity: 0; transform: translateY(15px); }
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-6 md:px-12 py-5 transition-all duration-300 ${scrolled ? (isDark ? 'bg-slate-950/95 border-slate-800' : 'bg-white/95 border-slate-200') + ' backdrop-blur-md border-b shadow-sm' : 'bg-transparent'}`}>
        <a href="#" className={`font-handwriting text-3xl md:text-5xl font-bold tracking-widest ${isDark ? 'text-white' : 'text-slate-900'} hover:opacity-70 transition`}>YL</a>
      ...
      <section className="min-h-screen flex flex-col justify-center px-6 md:px-12 relative pt-20 max-w-6xl mx-auto">
        <div className="space-y-8">
          ...
          <div className="space-y-6 max-w-4xl smooth-reveal font-light opacity-90" style={{animationDelay: '0.4s'}}>
            <h3 className="font-handwriting text-4xl md:text-5xl font-bold mb-4 mt-8">{t.hero.whoAmI}</h3>
            {t.hero.paragraphs.map((p, i) => (
              <p key={i} className="font-sans text-lg md:text-xl leading-relaxed text-slate-600 dark:text-slate-300">{p}</p>
            ))}
          </div>
      ...
      <section id="about" className={`py-32 ${isDark ? 'bg-slate-900/30' : 'bg-white'} border-y ${isDark ? 'border-slate-800' : 'border-slate-100'} px-6 md:px-12`}>
        <div className="max-w-6xl mx-auto">
          <h2 className="font-handwriting text-5xl md:text-6xl mb-16 text-center tracking-wide">{t.about.title}</h2>
          <div className="flex flex-col md:flex-row items-center gap-16 lg:gap-24">
            ...
            <div className="md:w-2/3 space-y-8">
              ...
              <div className="pt-8">
                <a href="/YANIV_LEVI_-_Senior_Product_Manager.PDF" download className={`inline-flex items-center gap-3 px-8 py-4 ${isDark ? 'bg-white text-slate-950 hover:bg-slate-200' : 'bg-slate-900 text-white hover:bg-slate-700'} rounded-lg font-sans font-bold tracking-widest uppercase text-xs transition shadow-xl`}>
      ...
      <section id="work" className="py-32 px-6 md:px-12 max-w-6xl mx-auto">
        <div className="mb-20 text-center">
          <h2 className="font-handwriting text-6xl md:text-7xl mb-6">Impact & Results</h2>
            ...
            <div key={project.id} className="group border-b border-slate-200 dark:border-slate-800 transition-all duration-500 cursor-default hover:bg-slate-500/5" onMouseEnter={() => setActiveProject(index)} onMouseLeave={() => setActiveProject(null)}>
              <div className="py-12 flex flex-col md:flex-row justify-between md:items-center gap-8 px-6">
                <div className="flex items-center gap-8 md:w-1/3">
                  ...
                  <div>
                    <h3 className="font-sans text-3xl font-bold tracking-tight">{project.company}</h3>
                    <p className="font-sans text-[10px] opacity-40 font-black tracking-[0.3em] mt-2 uppercase">{project.role}</p>
                  </div>
                </div>
                  ...
                  <div className="pt-10 border-t border-dashed border-slate-200 dark:border-slate-800 grid grid-cols-1 md:grid-cols-2 gap-16">
                    <div className="space-y-12">
                       <div>
                         ...
                         <p className={`font-sans text-lg leading-relaxed ${isDark ? 'bg-slate-800/30' : 'bg-[#FDFBF9]'} p-7 rounded-2xl border ${isDark ? 'border-slate-800' : 'border-slate-100'}`}>{project.challenge}</p>
                       </div>
                       <div>
                         <h4 className="font-sans font-bold text-[10px] uppercase tracking-[0.3em] opacity-30 mb-4 flex items-center gap-2"><Cpu size={14} /> {t.work.labels.solution}</h4>
                         <p className={`font-sans text-lg leading-relaxed ${isDark ? 'bg-slate-800/30' : 'bg-[#FDFBF9]'} p-7 rounded-2xl border ${isDark ? 'border-slate-800' : 'border-slate-100'}`}>{project.solution}</p>
                       </div>
                    </div>
                    <div className="space-y-12">
                      <div>
                        <h4 className="font-sans font-bold text-[10px] uppercase tracking-[0.3em] opacity-30 mb-4 flex items-center gap-2"><Plus size={14} /> {t.work.labels.impact}</h4>
                        <div className={`${isDark ? 'bg-white text-slate-950 shadow-white/5' : 'bg-slate-900 text-white shadow-black/20'} p-8 rounded-2xl shadow-2xl`}>
                          <p className="font-sans text-2xl font-semibold leading-snug italic tracking-tight">"{project.impact}"</p>
                        </div>
                      </div>
                      <div>
                        <h4 className="font-sans font-bold text-[10px] uppercase tracking-[0.3em] opacity-30 mb-6 flex items-center gap-2"><FileText size={14} /> {t.work.labels.artifacts}</h4>
                        <div className="flex flex-wrap gap-3">
                          {project.artifacts.map((art, i) => (
                            <span key={i} className={`flex items-center gap-2 px-4 py-2 ${isDark ? 'bg-slate-800 border-slate-700 text-slate-300' : 'bg-white border-slate-200 text-slate-600'} border rounded-full text-xs font-bold uppercase tracking-wider shadow-sm`}>
            ...
            {t.builder.projects.map((proj, idx) => (
              <div key={idx} className={`group flex flex-col md:flex-row md:items-center gap-6 md:gap-12 py-10 border-b ${isDark ? 'border-slate-800' : 'border-slate-200'} last:border-0 hover:bg-slate-500/5 transition-all duration-500 px-8 -mx-8 rounded-3xl`}>
                ...
                <div className="md:w-2/3 flex items-center justify-between gap-8">
                   <p className="font-sans text-lg opacity-60 group-hover:opacity-100 transition duration-500 leading-relaxed">{proj.desc}</p>
                   <a href="https://github.com/yanivle1-dotcom?tab=repositories" target="_blank" rel="noreferrer" className="opacity-20 hover:opacity-100 hover:scale-125 transition-all shrink-0">
          ...
          {t.experience.jobs.map((job, idx) => (
            <div key={idx} className="relative ps-12 group">
              ...
              <div className="flex flex-col md:flex-row md:items-baseline justify-between mb-4 gap-4">
                ...
                <span className={`font-sans text-[10px] font-black tracking-[0.2em] ${isDark ? 'bg-slate-800 text-slate-400' : 'bg-slate-100 text-slate-500'} px-4 py-2 rounded-full uppercase`}>{job.period}</span>
              </div>
              <p className="font-sans text-xl opacity-60 mt-4 leading-relaxed max-w-2xl">{job.desc}</p>
            </div>
          ))}
        </div>
      </section>
                  ...
                  <div className={`hidden md:flex w-full ${isEven ? 'justify-start' : 'justify-end'}`}>
                    <div className={`w-[44%] ${isEven ? 'text-start' : 'text-end'}`}>
                      <div className={`relative ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-100'} p-12 rounded-[2.5rem] border shadow-2xl group-hover:-translate-y-2 transition-all duration-500`}>
                        ...
                        <p className="font-sans text-lg opacity-60 leading-relaxed font-light">{step.desc}</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="w-full max-w-xl mb-24">
            {isSubmitted ? (
              <div className="flex flex-col items-center gap-4 py-12 animate-in fade-in zoom-in duration-500">
                ...
                <p className="font-sans text-xl font-bold">{t.footer.successMsg}</p>
              </div>
            ) : (
